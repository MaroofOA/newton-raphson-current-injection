# -*- coding: utf-8 -*-
"""
Created on Sat Oct  5 17:46:46 2024
...
from calculate_current_mismatch import calculate_current_mismatch
from build_current_Jacobian import build_current_Jacobian

# Define the function for the backward_forward method for distribution network analysis. 
def nrci_method(load_data, line_data, slack_bus_voltage=1, tolerance=1e-6, max_iter = 100, runs = 10):
    ...
    for run in range(runs):
        ...
        for iteration in range(max_iter):
            ...
            Ii_mismatch_imag = -I_mismatch[1:num_buses]  # Equivalent to MATLAB's (2:num_buses)
            Ii_mismatch_real = -I_mismatch[num_buses+1:]  # Equivalent to MATLAB's (num_buses+2:end)
            
            # Build the Jacobian matrix using the helper function
            current_Jacobian = build_current_Jacobian(num_buses, -P_load, -Q_load, v, Y_bus)
            
            # Solve for corrections
            mismatch = np.concatenate((Ii_mismatch_imag, Ii_mismatch_real))
            corrections = np.linalg.solve(current_Jacobian, mismatch)
            
            # Update the voltages
            V_mismatch_real = corrections[:num_buses-1]
            V_mismatch_imag = corrections[num_buses-1:]
            
            V_new_real = np.real(v[1:]) + V_mismatch_real
            V_new_imag = np.imag(v[1:]) + V_mismatch_imag
            
            # Update v with the new voltage values in rectangular form
            v[1:] = V_new_real + 1j * V_new_imag

            # End timing the iteration
            iter_time = time.time() - iter_start_time

            # Check for max_difference
            max_diff = np.max(np.abs(V_mismatch_real + 1j* V_mismatch_imag))
